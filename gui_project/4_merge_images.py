...
def merge_image():
    lst = list_file.get(0, END)
    images = [Image.open(i) for i in lst]

    imgs_width = [x.size[0] for x in images]   # →→→→→→→ width, height = zip(*(x.size for x in images))
    imgs_height = [x.size[1] for x in images]
    m_width = max(imgs_width)
    T_height = sum(imgs_height)
    ...
    save_path = os.path.join(txt_dest_path.get(), "Nado.png")
